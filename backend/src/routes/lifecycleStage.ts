...
const TABLE: Record<VType, Array<{
  ...
  sports: [
    ...
    { key:"Mint",  label:"안정",     color:"🟢", yearsMin:2,   yearsMax:3,   kmMin:30000,kmMax:60000, reason:"컨디션 유지 vs 사용강도에 따라 차", advice:"주요 소모품 점검" },
...
function pickStage(vt: VType, years: number, km: number) {
  ...
  for (const key of order) {
    ...
    if (!r) continue;
    const yearsOk = (years >= r.yearsMin) && (r.yearsMax === null ? true : years < r.yearsMax);
    const kmOk    = (km    >= r.kmMin)    && (r.kmMax    === null ? true : km    < r.kmMax);
    if (yearsOk && kmOk) return r;
  }
  return rows.find(x => x.key === "Blue")!;
}

router.get("/ping", (_req: Request, res: Response) => {
  res.json({ ok: true, msg: "ping from lifecycleStage" });
});
...
router.post("/stage", (req: Request, res: Response) => {
  ...
  if (typeof vehicleType !== "string" || typeof years !== "number" || typeof odometer !== "number") {
    return res.status(400).json({ error: "Expect { vehicleType, years, odometer }" });
  }
  ...
  res.json({
    ...
    meta: { version: "v0.1-rules", notes: "규칙 기반 MVP" }
  });
});
